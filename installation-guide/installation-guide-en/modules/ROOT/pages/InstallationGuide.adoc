...
* the user initiates access to the Kobee web application by navigating to `\http(s)://<host>:<port>/alm` (for example, `\http://kobee_server:8080/alm`).
...
* update the _kobee.properties_file_ to enable OAuth 2.0 SSO and configure the required properties for interaction with the registered Kobee application at the Idendity Provider.

=== Register the Kobee application with the Idendity Provider
...
* configure a *redirect URI* (4) as a _Web_ application type. Its value must point to the _callback_ endpoint of the Kobee web application, at the URL `\http://<host>:<port>/alm/callback`, for example, `\http://kobee_server:8080/alm/callback` or `\https://kobee.io/callback`

* in the _Token configuration_ tab, add the *groups* claim and ensure thti it isincluded in the _ID Token_, for example by emitting group values as _sAMAccountName_.
...
Next, uncomment the _kobee.oauth2.client-id_, _kobee.oauth2.client-secret_ and _kobee.oauth2.issuer-uri_ properties, and replace the values wwith the recorded *client-id*, *client-secret* and *tenant-id* when registrating Kobee as an app in the Idendity provider, see next sample with Microsoft Entra ID values.
...
When the _kobee.oauth2.issuer-uri_ has been set, Kobee will resolve the OpenID provider metadata, also known as the well-known openid-configuration. However it is still possible to overwrite these properties by uncommenting them in the kobee.properties file. Some other properties are not in the well-known openid-configuration, but have default values. They also can be overwritten by uncommenting them in the kobee.properties file. 
...
Navigate to the Kobee web application entry point at http(s)://<host>:<port>/alm (for example, http://kobee_server:8080/alm). The application redirects the user to an authentication request at the Identity Provider’s authorization endpoint. For Microsoft Entra ID, this endpoint is typically
