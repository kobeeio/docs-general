...
* l’utilisateur initie l’accès à l’application web Kobee en accédant à l’URL `\http(s)://<host>:<port>/alm` (par example, `\http://kobee_server:8080/alm`).
...
Accédez ensuite au point d’entrée de l’application web Kobee à l’adresse http(s)://<host>:<port>/alm (par exemple, http://kobee_server:8080/alm). L’application redirige alors l’utilisateur vers une requête d’authentification au niveau du point de terminaison d’autorisation du fournisseur d’identité. our Microsoft Entra ID, ce point de terminaison est généralement https://login.microsoftonline.com/{entra-tenant-id}/oauth2/v2.0/authorize,
