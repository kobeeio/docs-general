...
* Command Line and REST Interface enabling creation of builds and/or deploys on external triggers
...
* Execute a build or deploy from a command prompt/REST endpoint (C+R)
* Allows scheduling builds and deploys with an external scheduling tool (Task Scheduler, Cron, ...) (C+R)
* Generate a report from a command prompt/REST endpoint \(C)
* Create/edit a package from a command prompt/REST endpoint (C+R)
* Approve or Reject delivering a build to the next level \(R)
* Getting data about global setup, projects, builds and deploys,... in json or xml format \(R)
