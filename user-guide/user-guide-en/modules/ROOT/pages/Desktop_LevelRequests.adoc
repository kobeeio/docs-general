...
|2.
|The Monitor Process on the Kobee Server picks up the created Level Request and sends the required Pre- and Post-Notifications.
...
|8.
|As the Build Status is set to __Ready__, the Builder Agent on the Machine(s) where a Build must be executed, picks up this _Ready_ status and transports the Source Code from the Work Copy Location on the Kobee Server to the Build Environment Source Location, defined on this Machine and for this Build Level. 
...
If the Build Script is not found in the checked-out source code, the Builder searches the default Kobee Script Location as defined in the <<GlobAdm_System.adoc#_globadm_system_settings,System Settings>>. 
...
If the Build Script is not found here either, or if the transport process from the Kobee Script Location does not succeed, the Verify Build Script action fails and steps 9 and 10 are performed.

|12.
|If the Build Script Verification process succeeds, the Builder Agent executes the Build Script.

First, the Build Script is provided with the following parameters: Build Script Location, Source Location, Target Location, standard Kobee parameters and user-defined Build Parameters.
...
Depending on the Operating System of the Kobee server holding the Build Archive, a *.zip or *.tar.tgz will be created.
...
|15.
|The Builder Agent archives the Compressed Build to the Build Archive on the Kobee Server.

The Build Archive Location on the Kobee Server is defined in the <<GlobAdm_System.adoc#_globadm_system_settings,System Settings>>.
...
|22.
|If the Tagging Process is successful or if it was skipped because the Code was already tagged, the Monitor cleans up the used subdirectories in the Work Copy Location on the Kobee Server.
...
|23.
|The Monitor Process on the Kobee Server determines the final Level Request status.
...
|14.
|The Builder Agent compresses the Build artifacts on the Build Environment Target Location.
Depending on the Operating System of the Kobee Server holding the Build Archive, a *.zip or *.tar.tgz file will be created.
...
|22.
|If the Tagging Process is successful or if it was skipped because the Code was already tagged, the Monitor cleans up the used subdirectories of the Work Copy on the Kobee Server.
...
If the Deploy Script is not found in the decompressed Build result, the Deployer searches the default Kobee Script Location as defined in the <<GlobAdm_System.adoc#_globadm_system_settings,System Settings>>.
...
If the Build Script is not found here either or the transport from the Kobee Script Location does not succeed, the Verify Deploy Script action fails and steps 25 and 26 are performed.

|29.
|If the Verify Deploy Script process succeeds, the Deployer Agent executes the Deploy Script.

The Deploy Script is provided with the following parameters: Source Location, Target Location, standard Kobee Parameters and user-defined Deploy Parameters.
...
|32.
|As soon as the Monitor Process on the Kobee Server finds that all Deploy actions connected to a Level Request have the _Success_ or _Warning_ Status, it determines the final Level Request status.
...
|2.
|The Monitor Process on the Kobee Server picks up the Level Request and optionally sends the required Pre- and Post-Notifications.
...
If the Deploy Script is not found here either or the transport from the Kobee Script Location does not succeed, the Verify Deploy Script action fails and steps 7 and 8 are performed

|11.
|If the Verify Deploy Script process succeeds, the Deployer Agent executes the Deploy Script.

First the Deploy Script is provided with the following parameters: Source Location, Target Location, standard Kobee Parameters and user-defined Deploy Parameters.
...
|14.
|As soon as the Monitor Process on the Kobee Server detects a Deploy with Deploy Status _Success_ or __Warning__, it determines the final Level Request status.
...
* _Secured Parameters_ are non-editable parameters whose value cannot be read by any Kobee User.
+
...
* <<Desktop_LevelRequests.adoc#_desktop_lr_rssfeeds,The Kobee RSS Functionality>>
...
=== The Kobee RSS Functionality 
(((RSS Functionality))) 

If your Global Kobee Manager has activated RSS Feeds at System Settings level, the orange _RSS_ button is available on the _Search Level Request_ panel.
...
Kobee provides RSS Feeds for displaying data about the last 10 Level Requests that meet specified criteria.
...
+
You find a detailed explanation of the structure of the Kobee URL in the section <<App_RSS.adoc#_drssfeedurldetails,RSS URL Details>>.
...
+
More options are available when Generating a Report with the Kobee Command Line.
...
Issues can get linked to a Level Request in two ways: automatically by Kobee during the execution of the Level Request or manually by a User after the Level Request has ended.
...
Level Phases are handled by the Kobee Server.
...
|Retrieve Code
|The Kobee Server retrieves the source code from the VCR (and sometimes, in the case of certain dependencies, also the build results from the Build Archive) and stores it in a sub folder of the Work Copy Location so that the Agent can use it to perform a build.

|Build
|This Phase monitors the status of the Builds linked to the Level Request.
The Kobee Server notifies and does the follow-up of the IKAN ALM Agents that execute the Builds linked to the Level Request.

|Tag Code
|The Kobee Server tags the source code after a successful Build on a Build Level.
...
The Kobee Server notifies and does the follow-up of the IKAN ALM Agents that execute the Deploys linked to the Level Request.

|Link File Revisions
|This is a specific Phase for Levels in Package-based Projects, which is absent for Release-based Projects.

Based on the contents of the Package at the moment of the Level Request execution, the Kobee Server registers the checked-out versions extracted from the VCR for the Level.
...
|Cleanup Work Copy
|The Kobee Server cleans up the Work Copy folder where the source files (and other stored artifacts) have been checked out from the VCR in the _Retrieve
Code_ Phase.
|===
...
Build Phases are executed by the Kobee Agent while handling a Build.
...
|Transport Source
|The _Transport
Source Phase_ transports the required Source code from the Work Copy Location on the Kobee Server to the Build Environment Source Location.

It logs what was transferred and which Transporter was used in the _Message_ field.

|Verify Build Script
|The _Verify
Build Script Phase_ tries to locate the defined build script in the checked-out source code or in the Kobee Script Location.
...
|Archive Result
|The _Archive
Result Phase_ copies the compressed Build file to the Build Archive Location on the Kobee Server.
...
|The _Transport
Build Phase_ transports the Build from the Build Archive on the Kobee Server to the Deploy Environment Source Location.
...
|The _Verify
Deploy Script Phase_ tries to locate the defined deploy script in the decompressed Build result or in the Kobee Script Location.
...
This file is stored in the Kobee Build Archive Location.
...
To be able to download the Build file, the archive must be _Present_ in the Kobee database.
...
For some special events, like Level Request cancellations, abortions or failures, this reason is automatically provided by Kobee.
|===
...
Issues can get linked to a Level Request automatically by Kobee during the execution of the Level Request, or manually by a User after the Level Request has ended.
...
[NOTE]
====
Apart from the Issue ID, the information in the fields may not be automatically filled in when Kobee synchronizes the Issues with the Issue Tracking System.
...
MF ALM Defects, JIRA Issues, GitHub Issues and Team Foundation Artifacts may be linked to an Kobee Level Request, based on the comments provided by the developers when they commit their code to the VCR.
Kobee retrieves information from the corresponding Issue, such as a short description, the owner and the priority.
This information is synchronized during the Issue Tracking Phase each time the Level Request is delivered to the next Level in the Kobee Lifecycle.
...
This may be necessary when the automatic process did not detect some Issues, or when Issues need to be linked with Level Requests that were executed at a time when an external Issue Tracking System was not yet properly configured in Kobee.
...
For Builds based on latest code, these files were tagged by Kobee in the Tag Code Phase of this Level Request.
