...
For Release-based Projects, the rules for extracting files from the VCR are specified in the Project Stream definition by selecting the type of Build, and Kobee will automatically take care of the selection of the objects.
...
Refer to the section <<Desktop_Packages.adoc#_desktop_createpackage,Creating a Package>> for more information.
...
+
image::Desktop-Packages-PackagesOverview.png[,1071,525] 
+
. Use the search criteria on the _Search_ panel to only display the Packages you are looking for.
+
image::Desktop-Packages-PackagesOverview-SearchPanel.png[,983,99] 
+
...
This option is available to Kobee Users with Project User Access Rights.
...
+
image::Desktop-Packages-ViewPackage.png[,921,635] 
+
. Verify the settings.
+
...
+
Note that clicking the Clear button while holding down the CTRL key will select all files in the current tree.
* __Refresh__: to refresh the information on the information panels
...
image::Desktop-PackageDetails-Tab-Summary.png[,1146,607] 
...
image::Desktop-PackageDetails-Panel-Acttions.png[,311,247] 
...
|Status
a|This field contains the status of the Package. Possible statuses are Active, Frozen and Archived.
...
image::Desktop-PackageDetails-Panel-LatestLR.png[,900,176] 
...
image::Desktop-PackageDetails-Tab-ViewContents.png[,915,416] 
...
[NOTE]
====
If you do not select a specific revision of a file, the latest revision will be used at the moment the Package is built.
====
********* Deleting a File or Folder from the Package
+
...
[NOTE]
====
Files that have been deleted from the VCR, but are still linked to the Package, are displayed in red and with a strike-through font. The folders that contain those files are displayed with a a red exclamation mark after the folder's name. In this case, it is advised to also delete the removed files from the Package.
====
+
