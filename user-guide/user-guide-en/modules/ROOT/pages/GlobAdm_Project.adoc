...
[[_globadm_projectcreate]]
== Creating a Project 
(((Creating an Kobee Project)))  (((Creating an IKAN ALM Project ,Release-based)))  (((Creating an IKAN ALM Project ,Package-based)))  (((Projects ,Release-based)))  (((Projects ,Package-based)))  (((Creating an IKAN ALM Project))) 

There are 2 types of Projects:

* Release-based Projects: Kobee will work with the existing structure in the VCR (Version Control Repository) system, so that the objects to be extracted from the VCR will be retrieved automatically when starting the build process.
* Package-based Projects: Kobee introduces the Package concept, which enables to work with isolated files from the VCR system. Objects must be selected manually in a Package structure created in IKAN ALM before starting the Build process.
...
Other VCR types do not support working with isolated files and thus not with Packages in Kobee.
====
...
|VCR
|From the drop-down list, select the VCR to be used for this Kobee Project.

|VCR Project Name
|Enter the name of the VCR Project matching this Kobee Project.
It is possible that the VCR Project Name does not match the Kobee Project Name. 
...
If an Issue Tracking System is specified in this field, Kobee will always automatically create an Issue Tracking Level Phase when a new Level is created for the Project.

This field is only visible if external Issue Tracking Systems have been defined in Kobee.
...
|Build Script
|In this field, enter the name of the Build Script to be used for this Kobee Project.
...
|Deploy Script
|In this field, enter the name of the Deploy Script to be used for this Kobee Project.
...
|Locked
|This read-only field is set to __Yes__, because an Kobee Project can only be unlocked when is audited and fully configured.

|Hidden
|This read-only field is defaulted to __No__, because an Kobee Project cannot be hidden when it is still being created.
|===
+
...
|User Access
|From the drop-down list, select the User Group that will have User Access Rights for the new Kobee Project.
...
|Admin Access
|From the drop-down list, select the User Group that will have Administrator Access Rights for the new Kobee Project.
...
|Build Prefix
|Enter the Build Prefix to be used for the HEAD Project Stream of the new Kobee Project.

|Status
|Select the required status for the HEAD Project Stream of the new Kobee Project.

|Description
|Enter a description for the HEAD Project Stream of the new Kobee Project.

|Locked
|Select whether or not the HEAD Project Stream of the new Kobee Project must be locked.

By default, this option is set to __No__.

|Hidden
|This read-only field is defaulted to __No__, because an Kobee Project Stream cannot be hidden when it is still being created.

|Tag-Based
|When creating a Release-based Project, select whether or not the HEAD Project Stream of the new Kobee Project must be Tag-based.
...
|Accept Forced Build
|When creating a Release-based Project, select whether or not the HEAD Project Stream of the new Kobee Project must accept Forced Builds, i.e., allow Builds to bypass the Schedule set on the Build Level.
...
|Locked
|This read-only field is set to _No_, because a new Kobee Project is configured accordant to the cloned Project.
...
[NOTE]
====
When editing the Project export file, always change the Project name, since Kobee will refuse to import a Project when another Project with the same name already exists.
====
...
+
. Next, press the _Import_ button to import the uploaded project file into Kobee.
+
. When the import process is finished, Kobee will display either an info message (when successful) or an error message. The Validation Report in the Import Project panel provides more details about the result.
+
** Example: Info
+
