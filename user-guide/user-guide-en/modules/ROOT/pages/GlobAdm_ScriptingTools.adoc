...
Kobee supports the following Scripting Tools:
...
The following sections explain how to define a Scripting Tool so that the Kobee Agent may use it for Build and Deploy actions.

[NOTE]
====
You must always install the defined Scripting Tool on the Kobee Agent in order to execute Build and Deploy scripts.
====
...
If ANT is installed on a machine connected to your Build/Deploy Environments, Kobee will be able to interact with it.
Therefore, you must define the ANT Scripting Tool in the Kobee application and, when creating the Build or Deploy Environment, specify it as being the Build or Deploy Tool to be used.
...
Since Kobee launches ANT via Java, you must ensure that all required jar files are available.
...
The commandline options provided by default by Kobee, like -buildfile, -logfiles, -verbose, -debug ,... will not be accepted.
...
If Gradle is installed on a machine connected to your Build/Deploy Environments, Kobee will be able to interact with it.
Therefore, you must define the Gradle Scripting Tool in the Kobee application and, when creating the Build or Deploy Environment, specify it as being the Build or Deploy Tool to be used.
...
The commandline options which might be provided by default by Kobee will not be accepted: .-g, --gradle-user-home, -q, --quiet, -i, --info,-d, --debug, -s, --stacktrace, -S, --full-stacktrace, -b, --build-file
...
If NANT is installed on a machine connected to your Build/Deploy Environments, Kobee will be able to interact with it. 

Therefore, you must define the NANT Scripting Tool in the Kobee application and, when creating the Build or Deploy Environment, specify it as being the Build or Deploy Tool to be used.
...
If Maven2 is installed on a machine connected to your Build/Deploy Environments, Kobee will be able to interact with Maven2. 

Therefore, you must define the Maven2 Scripting Tool in the Kobee application and, when creating the Build or Deploy Environment, specify it as being the Build or Deploy Tool to be used.
...
[NOTE]
====
This option is only available if you have Global Admin Access Rights in Kobee.
====
...
Please note that you can NOT use the following options as they are already used by Kobee: "`-X`" or "`--debug`" (Debug), "`-s`" or "`--settings`" (Settings File), "`-P`" or "`--activate-profiles`" (Activate Profiles) and "`-B`" or "`--batch-mode`".
