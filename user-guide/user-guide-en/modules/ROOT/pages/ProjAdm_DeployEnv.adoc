...
If the option is activated, Kobee will analyze the differences between the two build results on the IKAN ALM Server and only new and modified files will be transported to the Agent.
...
When creating a Deploy Environment, Kobee will automatically link the default flow of Deploy Environment Phases to it.
