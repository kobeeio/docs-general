...
|2.
|Le processus Moniteur sur le serveur Kobee détecte la Requête de niveau créée et envoie les Pré- et les Post-notifications nécessaires.
...
|8.
|Dès que le statut de construction est mis à __Prêt__, l`'Agent de Construction sur la Machine ou les Machines où la Construction doit être créée, détecte ce statut et copie le code Source à partir de l`'Emplacement des Copies de travail sur le Serveur Kobee vers l`'emplacement Source de l`'Environnement de construction défini sur cette Machine et pour ce Niveau de construction.
...
Si le script de construction n`'est pas trouvé dans les codes Source contrôlés disponibles, l`'Agent de construction recherche l`'emplacement Kobee par défaut du script comme défini dans les <<GlobAdm_System.adoc#_globadm_system_settings,Paramètres Système>>.
...
Si le script de construction n`'est pas non plus trouvé dans cet emplacement, ou si le processus de transfert de l`'emplacement Kobee de script ne réussit pas, l`'action de vérification du script de construction échoue et les étapes 9 et 10 sont réalisées.

|12.
|Si le processus de vérification du script de construction réussit, l`'Agent de Construction l`'exécute.

D`'abord, le script de construction est alimenté avec les Paramètres suivants: l`'emplacement du script de construction, l`'emplacement de la Source, l`'emplacement de la Cible, les Paramètres Kobee standard et les Paramètres de construction définis par l`'utilisateur.
...
Suivant le système d`'exploitation du Serveur Kobee sur lequel se trouvent les Archives de Construction, un fichier _\*.zip_ ou _\*.tar.tgz_ sera créé.
...
|15.
|L`'Agent de Construction enregistre la Construction compressée dans les Archives de construction sur le serveur Kobee.

L`'Emplacement des Archives de construction sur le serveur Kobee est défini dans les <<GlobAdm_System.adoc#_globadm_system_settings,Paramètres Système>>.
...
|22.
|Si le processus de balisage réussit ou s`'il a été ignoré parce que le code était déjà balisé, le Moniteur libère les sous-répertoires utilisés dans la copie de travail sur le serveur Kobee.
...
|22.
|Si le processus de balisage du code réussit ou s`'il a été ignoré parce que le code était déjà balisé, le Moniteur libère les sous-répertoires utilisés de la copie de travail sur le serveur Kobee.
...
Si le script de déploiement n`'est pas trouvé dans le résultat de construction décompressé, l`'Agent de Déploiement recherche dans l`'emplacement des scripts Kobee par défaut comme défini dans les <<GlobAdm_System.adoc#_globadm_system_settings,Paramètres Système>>.
...
Si le script de déploiement n`'est pas non plus trouvé dans cet emplacement, ou si le processus de transfert de l`'emplacement Kobee de script ne réussit pas, l`'action de vérification du script de déploiement échoue et les étapes 25 et 26 sont réalisées.

|29.
|Si le processus de vérification du script de déploiement réussit, l`'Agent de Déploiement exécute le script.

Le script de déploiement contient les Paramètres suivants: l`'emplacement Source, l`'emplacement Cible, les Paramètres Kobee standard et les Paramètres de déploiement définis par l`'utilisateur.
...
Si le Script de déploiement n`'est pas trouvé dans le résultat de construction décompressé, l`'Agent de Déploiement recherche l`'emplacement Kobee par défaut de Script comme défini dans les <<GlobAdm_System.adoc#_globadm_system_settings,Paramètres Système>>.
...
Si le Script de déploiement n`'est pas non plus trouvé dans cet emplacement, ou si le processus de transfert de l`'emplacement Kobee de Script ne réussit pas, l`'action de vérification du Script de déploiement échoue et les étapes 7 et 8 sont réalisées.

|11.
|Si la vérification du Script de déploiement réussit, l`'Agent de Déploiement exécute le Script de déploiement.

D`'abord, le Script de déploiement est alimenté avec les Paramètres suivants: l`'emplacement Source, l`'emplacement Cible, les Paramètres Kobee standard et les Paramètres de déploiement définis par l`'utilisateur.
...
|14.
|Dès que le Moniteur sur le serveur Kobee détecte un Déploiement ayant le statut _Réussi_ ou __Avertissement__, il établit le statut final de la Requête de niveau:
...
* Les _Paramètres sécurisés_ sont des paramètres non-modifiables dont la valeur est illisible pour tous les utilisateurs Kobee.
+
...
Si l`'Administrateur global d`'Kobee a activé les flux RSS dans les Paramètres Système, le bouton RSS de couleur orange est disponible sur l`'écran __Rechercher des Requêtes de niveau__. 
...
Kobee offre des flux RSS assurant la visualisation de données concernant les 10 dernières Requêtes de niveau répondant aux critères spécifiés
...
+
Pour une description plus détaillée de la structure de l`'URL Kobee, se référer à l`'appendice <<App_RSS.adoc#_rssfeedurldetails,Détails de l`'URL du flux RSS>>.
...
Des Incidents peuvent être liés à une Requête de niveau selon l`'une des manières suivantes: automatiquement par Kobee lors de l'exécution de la Requête de niveau ou manuellement par l'Utilisateur quand la Requête de niveau s'est terminée.
...
Les Phases de niveau sont gérées par le Serveur Kobee.
...
Le Serveur Kobee notifie et assure le suivi des Agent IKAN ALM qui exécutent les Déploiements associés à la Requête de niveau.
...
En fonction du contenu du Paquet au moment de l`'exécution de la Requête de niveau, le Serveur Kobee enregistre les versions extraites du RCV pour ce Niveau.
...
|Le Serveur Kobee libère le Emplacement des Copies de travail à partir duquel le code Source a été récupéré du RCV dans la Phase __Récupération des Sources__.
|===
...
Les Phases de construction sont exécutées par l'Agent Kobee lors de l'exécution d'une Construction.
...
|La Phase _Transfert des Sources_ transfère le code Source requis à partir de l`'Emplacement des Copies de travail sur le serveur Kobee vers l`'emplacement Cible de l`'environnement de construction.
...
|La Phase _Vérification du script de construction_ essaie de localiser le script de construction dans le code Source récupéré ou dans l`'emplacement de script Kobee.
...
|Archivage Résultat
|La Phase _Archivage Résultat_ copie le fichier de construction compressé vers l`'Emplacement des Archives de construction sur le serveur Kobee.
...
|La Phase _Transfert du fichier de construction archivée_ transfère la Construction à partir de l`'Emplacement des Archives sur le serveur Kobee vers l`'emplacement Source de l`'Environnement de déploiement.
...
|La Phase _Vérification du script de déploiement_ essaie de localiser le script de déploiement défini dans le résultat de construction décompressé ou dans l`'emplacement Kobee des scripts.
...
Ce fichier est sauvegardé dans l`'emplacement Kobee des Archives de Constructions.
...
Pour pouvoir télécharger le fichier de Construction, les archives doivent être _présentes_ dans la Base de données Kobee.
...
Dans le cas d`'événements spéciaux, tels que les annulations, les interruptions ou les échecs d`'une Requête de niveau, ce commentaire est généré automatiquement par Kobee.
|===
...
Des Incidents peuvent être liés automatiquement à une Requête de niveau par Kobee lors de l`'exécution de la Requête de niveau.
...
[NOTE]
====
Outre l'Identifiant (ID) de l'Incident, l'information dans les champs n'est pas toujours saisie automatiquement quand Kobee synchronise les Incidents avec le Système de Suivi d'incidents.
...
Les Defects MicroFocus ALM, les Incidents JIRA, les Incidents GitHub et les Artefacts TeamForge et Team Foundation peuvent être reliés aux Requêtes de niveau de Kobee sur la base des commentaires fournis par les développeurs lors des processus d`'enregistrement (commit) des codes Source dans le RCV.
Kobee récupère des informations de l`'Incident correspondant, telles qu`'une courte description, le propriétaire et la priorité.
Ces informations seront synchronisées pendant la Phase de Suivi des Incidents chaque fois que la Requête de niveau sera délivrée au Niveau suivant dans le Cycle de vie Kobee.
...
Cela peut s`'avérer nécessaire quand le processus automatique n`'a pas détecté certaines Incidents, ou dans le cas où des Incidents doivent être liés à des Requêtes de niveau exécutées à un moment où le Système de Suivi des Incidents externe n`'était pas encore correctement configuré dans Kobee.
...
Pour les Constructions basées sur le dernier code, Kobee a balisé ces composants dans la Phase Balisage de cette Requête de niveau.
